...
function App() {
  ...
  useEffect(() => {
    if (!!searchGif) {
      let api = fetch(
        `https://api.giphy.com/v1/gifs/search?api_key=2ChuJQreZfShqfStn1d2SjzHXuBI7rT3&q=${search}&limit=12&offset=0&rating=g&lang=es`
      );
      api
        .then((response) => {
  ...
  useEffect(() => {
    if (!searchGif) {
      let autocomplete = fetch(
        `https://api.giphy.com/v1/gifs/search/tags?api_key=2ChuJQreZfShqfStn1d2SjzHXuBI7rT3&q=${search}&limit=4&offset=0&rating=g&lang=es`
      );
      autocomplete
        .then((response) => {
          return response.json();
